...
## 2026-09-24 — Graph Interop B2 metadata-only Torch Export

- Added a local JSON-only Torch Export proposal path using the public `TorchExportDocumentV1` / version 1 boundary. The Python extractor emits bounded architecture metadata only; trained tensor values are never serialized or imported. Explicit learner Apply creates the editable reference architecture without restoring weights, creating a trained model, or running it.
- Updated the strict V1 adapter, extractor, fixtures, tamper/version regressions, proposal preview limitations, and Graph Interop architecture documentation. The V1 fingerprint is cross-runtime canonical SHA-256 integrity metadata, not source authentication; `.pt2` loading remains an explicit trusted-local CLI operation.
- Acceptance: focused Torch Export interop/extractor, Graph Apply, Graph Interop, and Build Agent checks; full `npm run check`; production `npm run build`; six Chrome 153 browser scenarios across desktop, compact, Chinese, and parallel-language modes; and `git diff --check` passed. VOLK-Dev issued terminal PASS for `4ed779b087c5ee6cd061ba9c8323989784571d33`; the subsequent changelog-only delivery is based on that accepted code.
- Limitations: real PyTorch ExportedProgram integration was skipped because PyTorch is not installed; no dependency was installed. The production build retains the existing greater-than-500-kB chunk advisory. Imported graphs preserve architecture only; original Python structure, trained parameter values, and dynamic batch constraints are not restored.

## 2026-09-25 — Torch Export PyTorch 2.14 output metadata repair

- Preserved the public metadata-only `TorchExportDocumentV1` contract while deriving a single USER_OUTPUT's dtype and shape from its referenced FX producer when the output wrapper has no `val`. Multiple outputs and invalid tensor metadata remain rejected.
- Added a real `torch.export.export` regression through extraction, V1 validation, and proposal creation; verified output float32 shape `(2,4)`, the 8→32→4 Dense mapping, no parameter-value sentinel leakage, and rejection of the unsupported `sin` operator. With an explicit `PYTHON` override, missing PyTorch now fails this real-runtime regression rather than silently skipping.
- Acceptance: focused Torch Export interop/extractor, Graph Apply, Graph Interop, and Build Agent checks; full `npm run check` with Python 3.12.10 and PyTorch 2.14.0+cpu; production `npm run build`; six Chrome 153 browser scenarios; `git diff --check`; and independent VOLK-Dev PASS for `aeac42016befbadee93bbe5b40b2fa955960d2a0`.
- Limitations: the real integration was verified on PyTorch 2.14.0+cpu; no minimum or maximum supported PyTorch range is declared. The production build retains the existing greater-than-500-kB chunk advisory. Tensor values remain intentionally unimported.

## 2026-09-25 — Graph Infrastructure B3 local ONNX import

- Added a bounded local ONNX ModelProto normalizer for a strict metadata-only `VolkOnnxDocumentV1`, plus deterministic opset-13 graph materialization into the existing detached proposal, read-only preview, current-state revalidation, and learner-confirmed Apply path. Build More accepts normalized JSON only; trained tensor values are never imported.
...
## 2026-09-25 — C2 occupied-workspace graph patch preview and Apply

- Learners can now review a read-only, localized before/after diff for a detached graph patch and explicitly Apply it to an occupied Build workspace. The preview groups node and connection changes, shows parameter/layout/endpoint details and ordered operations, and blocks stale or otherwise invalid patches without overwriting newer edits.
- Added the occupied-workspace preparation/commit boundary to the existing source-neutral proposal submission path. Apply revalidates the canonical project, current component registry, runtime, and workspace snapshot; semantic changes clear trained-model/run results, while layout-only moves preserve them. Whole-graph B1 Apply remains empty-workspace-only.
- Added deterministic occupied-graph pure/browser coverage and updated graph interoperability architecture documentation. Affected areas: `src/core/graph/workspacePatchApply.js`, `src/components/graph/GraphPatchPreview.jsx`, `src/main.jsx`, `src/locales/ui.js`, `src/components/graph/GraphApplyBrowserTestBridge.jsx`, graph check scripts, `package.json`, and `docs/architecture/`.
- Acceptance: independent VOLK-Dev PASS for the accepted source fingerprint; full `npm run check` with real PyTorch 2.14.0+cpu and ONNX 1.23.0, `npm run build`, C2 Graph Patch Apply, B1 Graph Apply and B2 Torch Export browser regressions, real-fixture B3 ONNX browser regression, localization/browser matrix, and `git diff --check` passed. The build retains the existing large-chunk advisory.
- Limitations: `REPLACE_SUBGRAPH` remains unsupported in GraphPatchProposalV1; unsupported operations are rejected before preview. Apply remains an explicit learner action, not an agent or Canvas Agent mutation capability.
