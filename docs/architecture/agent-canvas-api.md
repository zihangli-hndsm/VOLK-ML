...
Dataset replacement accepts the same tabular dataset shape used by project files: non-empty row objects, feature-column names, and a target-column name. The adapter derives normalized column metadata from the supplied rows before exposing the dataset to the editor. It does not upload data.
