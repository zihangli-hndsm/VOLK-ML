...
- Goal taxonomy + fidelity (PR E.2 / E.2.1): `inspectContext()` exposes `teaching.objectives` (the bounded pedagogical vocabulary), `teaching.supportedObjectives` (capability-grounded) and `teaching.capabilities` / `teachingCapabilities` (the model's declared teaching-evidence contract). Normalized goals carry a semantic `objective` alongside the E.1 goal family, and unsupported objectives reject with `TEACHING_GOAL_UNSUPPORTED` instead of being reinterpreted. `composeScript(plan)` additionally runs goal fidelity evaluation and returns `{ mode: 'composed', plan, script, fidelity, dryRun }`. Since PR E.2.1 fidelity is outcome-truthful: requirements use three explicit evidence classes (`visualEvidence` = the Script's concrete `primitive.props` actually bind the path; `runtimeEvidence` = the replayed semantic state contains the result; `traceEvidence` = required events fired, including payload predicates such as `training.completed{stoppedReason=learning-rate-too-high}`), so "training happened" is never treated as "failure happened". `show_failure_case` currently targets the inspectable learning-rate-too-high stopped regime (loss + gradient + parameter-movement evidence exist); the raw early non-finite `diverged` stop reason remains valid runtime behavior but is not advertised by the capability until a future contract can explain it truthfully. Qualitative learning-rate-too-high text produces a semantic probe (`direction: 'increase'`), and the Planner derives the numeric value from current controls + `controlSchemas`; no lexical numeric constant. A technically valid script that fails the teaching goal throws `TEACHING_GOAL_FIDELITY_FAILED` with structured `missing` requirements.
- Toolkit expansion + MLP (PR F.1): the primitive toolkit adds `parameter-trajectory`, `network-graph`, `matrix-grid` and `histogram` (typed props, declarative placement/whenControl, compatible bindings). A new `mlp` model adapter and `mlp-classification` playground register through the same model/playground registries: `inspectContext()` exposes the MLP semantic schema, operation intents (`fit`/`predict`), trace schemas and declared teaching capabilities (`show_training` + `explain_prediction`; `show_failure_case` honestly rejected). The generic `plan` -> `composeScript` -> fidelity pipeline handles MLP goals (`Explain this MLP prediction`, `show_training`, compare `hiddenUnits`, what-if `learningRate`) with `mode: 'composed'` and zero model branches in the Script Runtime / Materializer / Composer / fidelity evaluator / unified stage.
- Agent UI + script tooling (PR F.2): the Playground dialog gains an Agent panel (Ask Agent -> plan -> compose -> preview with Overview / Teaching Plan / Visualization Script / Fidelity tabs; Run only loads and plays through the existing Script Runtime), plus Copy JSON / Download JSON / Load JSON (Load always passes validate -> model compatibility -> strict dry run). Host-derived snapshots now carry `provenance` (`preset` / `generated` / `composed` / `revised` / `imported`) so the UI can distinguish Preset / Composed by Agent / Imported Script. A new bounded `canvas.playground.reviseScript({ plan, script, request })` API transforms an existing TeachingPlan + Script with typed revisions (`shorten`, `remove_visual`, `keep_visuals`, `focus_result`, `change_comparison_values`); every revision is validated, strict-dry-run and goal-fidelity-checked, and revisions that would destroy the teaching goal reject with `TEACHING_GOAL_FIDELITY_FAILED` instead of silently producing a misleading script.
