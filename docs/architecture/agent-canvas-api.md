...
- `plan(goal)` (PR E.1 / E.1.1) converts a teaching goal into a JSON-safe **TeachingPlan v1**. Text goals go through the lexical parser first (`parseTeachingGoalText`), then the schema-grounded planner validates everything against the same `inspectContext()` the Agent reads: comparison values and what-if values are checked against `controlSchemas`, run objectives come from the control's declarative `runObjective`, operations come from `context.model.operations` by `intent`, and reveal counts come from `playback.revealCountControl`. An explicit request for an unavailable control (e.g. `k=1 和 k=15` on LR) rejects with `TEACHING_CONTROL_INVALID` instead of being reinterpreted. `diagnose` is rejected with `TEACHING_GOAL_UNSUPPORTED` (not implemented). Pairwise comparison is the v1 cardinality: exactly two values, no silent defaults. Stable codes: `TEACHING_GOAL_UNSUPPORTED`, `TEACHING_CONTROL_INVALID`, `TEACHING_VALUE_OUT_OF_RANGE`, `TEACHING_PLAN_INVALID`.
- `composeScript(plan)` (PR E.1.1) implements the deterministic Composer: TeachingPlan -> Visualization Script. The Composer **iterates/compiles `plan.phases`** (typed `observe`/`set-control`/`run`/`reveal`/`capture`/`restore`/`summarize` phases); it never regenerates the sequence from `goal.type`. Primitives are discovered generically from the PR D primitive schemas (`compatibleBindings` + declarative `placement`/`whenControl`), run operations are looked up by `intent` (never by names like `traceFit`/`tracePredict`), and there are no model-shaped templates. The plan is validated against the current context before composition (`validatePlanAgainstContext`), so a KNN plan composed in an LR session fails early with `TEACHING_PLAN_INVALID` (playground mismatch). The composed script is validated and strict-dry-run against the live session before it is returned; the caller then `loadScript()`s it. Comparison plans capture semantic snapshots (`capture`/`restoreCapture` steps, including timeline and trace checkpoints) so k=1 vs k=15 experiments are deterministic, replayable, and each capture holds **completed** prediction/voting evidence rather than a reset state.
