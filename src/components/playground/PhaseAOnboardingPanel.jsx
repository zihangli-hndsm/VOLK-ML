export default function PhaseAOnboardingPanel({ snapshot, host, onDispatch, onOpenWorldTools, t }) {
  ...
  const hasLearnerAction = Boolean(snapshot?.orchestrationRuntime?.facts?.meaningfulLearnerAction);
  ...
  const orchestrationQuestion = snapshot?.orchestrationRuntime?.stageId === 'BASELINE'
    ? 'episode.zero.question'
    : snapshot?.orchestrationRuntime?.currentQuestion;
  return <section data-phase-a-onboarding data-orchestration-stage={snapshot?.orchestrationRuntime?.stageId ?? undefined} className="rounded-2xl border border-amber-100 bg-amber-50/70 p-3 text-sm text-slate-800">
