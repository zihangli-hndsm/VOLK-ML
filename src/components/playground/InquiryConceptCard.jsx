...
export default function InquiryConceptCard({ card, onDismiss, onOpenEvidence, t }) {
  ...
  return <section data-ui-inquiry-concept-card="true" className="rounded-2xl border border-blue-200 bg-blue-50/70 p-3 text-sm text-blue-950" aria-label={t('playground.inquiry.card.ariaLabel')}>
    <div className="flex items-start justify-between gap-3">
      ...
      <button type="button" aria-label={t('playground.inquiry.card.dismiss')} onClick={onDismiss} className="ui-motion-interactive min-h-10 rounded-xl px-2 text-xs font-black text-blue-800 hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-500">{t('playground.inquiry.card.dismiss')}</button>
    </div>
    <p className="mt-2 text-xs">{t(card.summaryKey)}</p>
    <p className="mt-3 text-[10px] font-black uppercase tracking-wide text-blue-700">{t('playground.inquiry.card.whyNow')}</p>
