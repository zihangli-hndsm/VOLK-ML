...
export default function ExperimentBar({ snapshot, onDispatch, t, highlightedAffordances = [] }) {
    ...
    return <section data-experiment-compact-initial="true" className="min-w-0 rounded-2xl border border-slate-200 bg-white px-3 py-2" aria-label={t('playground.experiment.ariaLabel')}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <button type="button" aria-expanded={moreOpen} aria-controls="experiment-more-actions" onClick={() => setMoreOpen((value) => !value)} className="rounded-xl px-2 py-2 text-lg font-black leading-none text-slate-500 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label={t('playground.experiment.moreActions')}>•••</button>
        </div>
      </div>
  ...
  return <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-3" aria-label={t('playground.experiment.ariaLabel')}>
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      <div className="flex items-center gap-1">
        ...
        <button type="button" aria-expanded={moreOpen} aria-controls="experiment-more-actions" onClick={() => setMoreOpen((value) => !value)} className="rounded-xl px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500">{t('playground.experiment.moreActions')}</button>
      </div>
    </div>
    ...
    {comparison.enabled && diff && <div className="mt-3 space-y-3">
