...
export default function ExploreDetailsRegion({ snapshot, modelPlayground, bigIdea, agent, host, activeDepth, onDepthChange, agentOpen, onAgentOpen, onAgentClose, onDispatch, onGuidanceChange, formulaPrimitive, onOpenWorldTools, initialSelection, onAskAboutSelection, illuminatedConceptIds = [], journeyIlluminationEvents = [], onIlluminateConcept, learningPathIlluminatedIds = [], onIlluminateLearningPath, hypotheses = [], evidenceInstances = [], onCreateHypothesis, onSetHypothesisStatus, onAttachHypothesisEvidence, onOpenHypothesisEvidence, testDesigns = [], testDesignResults = {}, testDesignCapabilities = null, onSaveTestDesign, onRunTestDesign, hypothesisGroups = [], discriminationPlans = [], onCreateHypothesisGroup, onCreateDiscriminationPlan, interpretations = [], revisions = [], onCreateInterpretation, onCreateRevision, counterfactualQuestions = [], onCreateCounterfactual, onConvertCounterfactual, counterfactualConditionFingerprint = null, onAcceptLumiSuggestion, onRequestLifecycle, presentation = null, onPresentationFeedbackConsumed, resolvedLumiTarget = null, lumiGuidanceDismissed = false, onDismissLumiGuidance, t, intervention = null }) {
  ...
  const lumiEvidenceStatus = snapshot?.inquiryRuntime?.evidence?.status ?? 'insufficient';
  const lumiMeaningfulResult = ['valid-weak', 'evidenced'].includes(lumiEvidenceStatus);
  ...
  return <section data-ui-region="details-region" data-ui-layer={activeDepth === CONCEPTUAL_DEPTHS.TUNE ? 'tune' : activeDepth ? 'inspect' : 'play'} className="relative min-w-0 space-y-3" aria-label={t('playground.depth.regionLabel')}>
    ...
    {agent && <div data-lumi-ambient="true"><LumiCompanion snapshot={snapshot} attention={attention} compact={compact} semanticAction={lumiSemanticAction} semanticTarget={lumiSemanticTarget} recentConceptEvent={lumiRecentConceptEvent} meaningfulResult={lumiMeaningfulResult} askBusy={agentBusy} presentation={presentation} resolvedTarget={resolvedLumiTarget ?? intervention?.resolvedTarget ?? null} guidanceDismissed={lumiGuidanceDismissed} onDismissGuidance={onDismissLumiGuidance} onPresentationFeedbackConsumed={onPresentationFeedbackConsumed} onOpenGuidance={onAgentOpen} onOpenEvidence={() => onDepthChange?.(CONCEPTUAL_DEPTHS.EVIDENCE)} onOpenIdeas={() => { const node = document.querySelector('[data-secondary-inquiry-surfaces]'); if (node) { node.open = true; node.scrollIntoView?.({ behavior: 'smooth', block: 'center' }); } }} onSelectContinuation={(id) => host?.recordInquiryContinuation?.(id)} onOpenSettings={openSettings} isConfigured={isConfigured} configureLabel={t('ai.configure')} t={t} /></div>}
    ...
    {agentOpen && <div id="explore-agent-guide"><ExploreAgentSurface snapshot={snapshot} agent={agent} capabilities={capabilities} compact={compact} onClose={onAgentClose} onDepthChange={onDepthChange} onOpenAiSettings={openSettings} host={host} closeRef={agentCloseRef} initialSelection={initialSelection} onAskAboutSelection={onAskAboutSelection} illuminatedConceptIds={illuminatedConceptIds} onIlluminateConcept={onIlluminateConcept} onBusyChange={setAgentBusy} onRequestLifecycle={onRequestLifecycle} t={t} /></div>}
