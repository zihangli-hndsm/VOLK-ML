...
export default function ExploreContextBar({ playground, snapshot, onDispatch, onPresent, onClose, t, highlightedAffordances = [], phenomenon = false }) {
  ...
  return <header data-ui-region="context-bar" aria-label={t('playground.explore.contextBarLabel')} className="flex min-w-0 flex-wrap items-start justify-between gap-3 border-b border-slate-200 pb-4">
    ...
    <div className={`flex shrink-0 items-center gap-2${compact ? ' ml-auto' : ''}`}>
      ...
      <div className="relative">
        <button type="button" aria-expanded={moreOpen} aria-controls="explore-more-actions" onClick={() => setMoreOpen((open) => !open)} className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500">{t('playground.explore.more')}</button>
        {moreOpen && <div id="explore-more-actions" className="absolute right-0 z-10 mt-2 grid min-w-52 max-w-[calc(100vw-1.5rem)] gap-1 rounded-2xl border border-slate-200 bg-white p-2 text-left shadow-xl">
