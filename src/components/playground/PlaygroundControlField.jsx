export default function PlaygroundControlField({ control, snapshot, onDispatch, t, showHint = false, changed = false, held = false, compact = false }) {
  ...
  if (control.type === 'boolean') {
    return <label className={`flex items-center justify-between gap-3 ${surface}`}>
  ...
  if (control.type === 'select') {
    ...
    return <label className={`block ${surface}`}>
      {header}{hint}
