export default function FormulaRenderer({ props, t }) {
  ...
  if (formula.key === 'playground.formula.linear') {
    return <p className="font-mono text-sm font-bold text-sky-300" style={{ opacity: props.motionOpacity ?? 1 }}>
  ...
  return <p className="font-mono text-sm font-bold text-sky-300" style={{ opacity: props.motionOpacity ?? 1 }}>{t(formula.key, formula.params)}</p>;
}
