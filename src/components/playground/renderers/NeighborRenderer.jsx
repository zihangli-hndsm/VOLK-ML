  ...
  return <>
    {selected.map((neighbor) => {
      ...
      return <React.Fragment key={`neighbor-${neighbor.pointId}`}>
        ...
        <line x1={xToSvg(query.x)} y1={yToSvg(query.y)} x2={xToSvg(point.x)} y2={yToSvg(point.y)} stroke={props.highlighted ? '#f59e0b' : colorByLabel?.[neighbor.label] ?? '#94a3b8'} strokeWidth={props.highlighted ? 4 : 2} opacity="0.85" />
      </React.Fragment>;
    })}
    {selected.map((neighbor) => {
      const point = points.find((item) => item.id === neighbor.pointId);
