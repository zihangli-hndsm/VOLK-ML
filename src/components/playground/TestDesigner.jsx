...
export default function TestDesigner({ hypothesis, snapshot, capabilities, t, onSave, onCancel, existingDesign = null }) {
  ...
  const [held, setHeld] = useState(existingDesign?.heldConstantFactors ?? []);
  ...
  return <section data-test-designer="true" className="test-designer mt-3 rounded-xl border border-orange-200 bg-orange-50/60 p-3">
    ...
    <fieldset className="mt-3">
      ...
      <div className="mt-1 grid grid-cols-2 gap-1 sm:grid-cols-3">{(capabilities?.heldOptions ?? []).map((id) => <label key={id} className="flex min-w-0 items-center gap-1 rounded-lg bg-white px-2 py-1 text-[10px] text-slate-700"><input type="checkbox" checked={held.includes(id)} onChange={() => toggle(setHeld, id)} className="accent-orange-500" /><span className="truncate">{t(`playground.experiment.factor.${id}`, { defaultValue: id })}</span></label>)}</div>
    </fieldset>
    <fieldset className="mt-3">
    ...
    {error && <p role="alert" className="mt-2 text-[11px] font-bold text-red-700">{t(`playground.testDesign.error.${error}`, { defaultValue: error })}</p>}
