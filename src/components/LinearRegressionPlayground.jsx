...
export default function LinearRegressionPlayground({ dataset, t }) {
  ...
  return <section className="mt-6 rounded-3xl border border-blue-200 bg-blue-50/70 p-4 sm:p-5">
    ...
    <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="font-mono text-sm font-black text-slate-800">ŷ = {numberLabel(weight)}x {bias < 0 ? '−' : '+'} {numberLabel(Math.abs(bias))}</p><button onClick={setBestFit} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white hover:bg-blue-700">{t('playground.bestFit')}</button></div>
  </section>;
}
