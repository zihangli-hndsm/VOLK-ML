...
function account(value) { if (value == null) return null; object(value, 'account-shape'); return Object.freeze({ id: boundedText(value.id, 120), email: boundedText(value.email, 240), role: boundedText(value.role, 80), createdAt: boundedDate(value.createdAt) }); }
...
export function normalizeCloudLoginResponse(payload) { object(payload, 'login-shape'); if (typeof payload.accessToken !== 'string' || !payload.accessToken.trim()) throw contractError('access-token-shape'); if (payload.tokenType !== 'bearer') throw contractError('token-type'); return Object.freeze({ apiVersion: CLOUD_AUTH_API_VERSION, accessToken: payload.accessToken.trim().slice(0, 4096), tokenType: 'bearer', account: account(payload.account) }); }
