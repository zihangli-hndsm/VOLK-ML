...
export function analyzeBrowserExecutionGraph({ nodes, edges, dataset, alreadyFlattened = false }) {
  ...
  if (roots.length > 1) return failure('error.multipleTrainingRoots', flattened, roots.map((node) => node.id));
  const root = roots[0];
  ...
  if (root.data.manifest.op === 'knn_classifier') {
    ...
    if (!requireSource(root, 'dataset', 'tabular_data')) return failure('error.invalidConnection', flattened, [root.id]);
    ...
    if (!linear || !requireData(requireSource(linear, 'split', 'train_test_split'))) return failure('error.invalidConnection', flattened, [root.id]);
  } else {
    ...
    if (!split || !output || !loss || !optimizer || !requireData(split)) return failure('error.invalidConnection', flattened, [root.id]);
    allowed.add(loss.id);
    ...
    if (!['mse_loss', 'cross_entropy_loss'].includes(loss.data.manifest.op) || !['sgd_optimizer', 'adam_optimizer'].includes(optimizer.data.manifest.op)) {
      return failure('error.backendMissing', flattened, [root.id]);
    }
    const architecture = [];
    const seen = new Set();
    let current = output;
    while (current.data.manifest.op !== 'tensor_input') {
      if (seen.has(current.id)) return failure('error.pipelineCycle', flattened, [current.id]);
      seen.add(current.id);
      const op = current.data.manifest.op;
      if (!['model_output', 'dense', ...activationOps].includes(op)) return failure('error.browserMlpArchitecture', flattened, [current.id]);
      architecture.push(current);
      allowed.add(current.id);
      const edge = incoming(current, 'input')[0];
      current = nodeById.get(edge?.source);
      if (!current) return failure('error.browserMlpArchitecture', flattened, [root.id]);
    }
    allowed.add(current.id);
    ...
    if (dimensions.length !== 1 || !Number.isInteger(dimensions[0]) || dimensions[0] <= 0 || dimensions[0] !== featureCount || current.data.parameters.dtype === 'int32') {
      return failure('error.browserMlpShape', flattened, [current.id]);
    }
    ...
    if (!dense.length) return failure('error.browserMlpArchitecture', flattened, [root.id]);
    for (const node of layers) {
      if (node.data.manifest.op === 'softmax' && Number(node.data.parameters.axis) !== -1) {
        return failure('error.browserMlpArchitecture', flattened, [node.id]);
      }
      ...
      if (!Number.isInteger(inputFeatures) || inputFeatures !== width || !Number.isInteger(units) || units <= 0) return failure('error.browserMlpShape', flattened, [node.id]);
      width = units;
    }
    ...
    if (classification) {
      if (classCount < 2 || !classificationSplitHasTest || loss.data.manifest.op !== 'cross_entropy_loss' || finalOp !== 'softmax' || width !== classCount) {
        return failure('error.browserMlpClassification', flattened, [root.id]);
      }
    } else if (loss.data.manifest.op !== 'mse_loss' || width !== 1 || finalOp === 'softmax') {
      return failure('error.browserMlpRegression', flattened, [root.id]);
    }
  }

  for (const edge of outgoing(root)) {
    const consumer = nodeById.get(edge.target);
    if (!consumer) return failure('error.invalidConnection', flattened, [root.id]);
    ...
    if (!allowedConsumer) return failure('error.wrongEvaluator', flattened, [consumer.id]);
    allowed.add(consumer.id);
  }
  if (activeNodes.some((node) => !allowed.has(node.id))) return failure(
    'error.invalidConnection',
    flattened,
    activeNodes.filter((node) => !allowed.has(node.id)).map((node) => node.id),
  );
