...
function localPedagogicalDesign(text, snapshot) {
  ...
  if (/(overlap|classes overlap|class overlap|move .*class.*closer|classes? .*closer|less separated|move .*class|重叠|类别.*重叠|类.*重合|类别.*(靠近|接近)|类.*(靠近|接近))/i.test(text) && task === 'classification') {
