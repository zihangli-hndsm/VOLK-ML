...
export function teachingDialoguePrompt(context) {
  return [
    ...
    'Move/content compatibility is strict: ELICIT_PREDICTION uses prediction, ASK_FOR_REASON uses reason, OFFER_HINT uses hint, REQUEST_TEACH_BACK uses teachBack, and SUMMARIZE_AND_PAUSE uses summary with current evidenceRefs.',
    'EXPLAIN_WITH_EVIDENCE uses conceptual only when measured evidence is unavailable; otherwise it must use the current observed variant and supplied evidenceRefs. A measured or summary claim is invalid without current evidence, a World-held comparison, sampling-only changed factors, and an available line-movement outcome.',
