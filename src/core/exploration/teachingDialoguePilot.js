...
export function teachingDialoguePrompt(context) {
  return [
    ...
    'content must be exactly {"key":"episode.one.teachingDialogue.hint","params":{}} (the params object may be empty or omitted). Never add content.text, prose, markdown, or any other content key; content.text is invalid and must not be used as a substitute for content.key.',
    ...
    'Move/content compatibility is strict. Exact move/content mapping is strict: ELICIT_PREDICTION -> episode.one.teachingDialogue.prediction; ASK_FOR_REASON -> episode.one.teachingDialogue.reason; OFFER_HINT -> episode.one.teachingDialogue.hint; REQUEST_TEACH_BACK -> episode.one.teachingDialogue.teachBack; SUMMARIZE_AND_PAUSE -> episode.one.teachingDialogue.summary with current evidenceRefs.',
    'EXPLAIN_WITH_EVIDENCE -> episode.one.teachingDialogue.evidence when the supplied evidence is evidenced and line movement is visible; -> episode.one.teachingDialogue.observedWeak only when the supplied line movement is weak or valid-weak; -> episode.one.teachingDialogue.observedUnchanged only when activeComparison.outcome or the supplied line-movement fact is exactly unchanged; otherwise use episode.one.teachingDialogue.conceptual only when measured evidence is unavailable. Never use observedUnchanged for a visible context. A measured or summary claim is invalid without current evidence, a World-held comparison, sampling-only changed factors, and an available line-movement outcome.',
...
export function createTeachingDialogueResponse({ context, move, contentKey = contentKeyFor(move), questionRef = null, statementRefs = [], evidenceRefs, provisionalHypothesis = null, expectedReplyKind = expectedReplyFor(move), grounding = null, origin = 'local', fallbackReason = null } = {}) {
  ...
  return {
    ...
    expectedReplyKind,
    content: { key: TEACHING_DIALOGUE_CONTENT_KEYS.includes(safeContentKey) ? safeContentKey : TEACHING_DIALOGUE_CONTENT_KEYS[2], params: {} },
    grounding: safeGrounding,
    origin: TEACHING_DIALOGUE_ORIGINS.includes(origin) ? origin : 'local',
    fallbackReason: origin === 'fallback' && TEACHING_DIALOGUE_FAILURE_REASONS.includes(fallbackReason) ? fallbackReason : null,
  };
}
