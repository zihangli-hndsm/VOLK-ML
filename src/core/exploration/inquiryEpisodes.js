...
export function deriveInquiryTrailEntries({ semanticEvents = [], journey = null, inquiry = null, hypotheses = [], testDesigns = [], interpretations = [], revisions = [], counterfactualQuestions = [], illuminationEvents = [] } = {}) {
  ...
  (Array.isArray(revisions) ? revisions : []).slice(0, 12).forEach((item, index) => entries.push(createTrailEntry({ type: 'revise', id: `trail-revision-${item.childHypothesisId}`, sourceIds: [item.parentHypothesisId, item.childHypothesisId, ...(item.interpretationIds ?? [])], sequence: 200 + index })));
