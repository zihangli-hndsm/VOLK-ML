...
export function localFallbackPolicy(context = {}) {
  ...
  if (context.nextOperation === 'RESAMPLE_WORLD') return { version: 1, type: 'SUGGEST_EXPERIMENT', payload: { operation: 'RESAMPLE_WORLD', reason: 'same-world-resample' }, authority: 'suggestion-only', requiresLearnerAcceptance: true };
  if (context.nextOperation === 'RUN') return { version: 1, type: 'SUGGEST_EXPERIMENT', payload: { operation: 'RUN', reason: context.inquiryRuntime?.baseline?.fit ? 'fit-active-branch' : 'capture-baseline' }, authority: 'suggestion-only', requiresLearnerAcceptance: true };
  if (context.nextOperation === 'SET_COMPARE') return { version: 1, type: 'SUGGEST_EXPERIMENT', payload: { operation: 'SET_COMPARE', reason: 'compare-evidence' }, authority: 'suggestion-only', requiresLearnerAcceptance: true };
...
export function applyGuidanceBudget(history = [], action, { stage = 'unknown', conceptSurfaced = false, dismissed = false } = {}) {
  ...
  const canonical = validateLumiAction(action).action ?? staySilent();
  // Silence is a policy outcome, not a consumed proactive hint. It must not
  // permanently exhaust a stage when a later semantic transition is eligible.
  if (canonical.type === 'STAY_SILENT') return bounded;
  if (previous.some((entry) => entry?.action?.type === canonical.type)) return bounded;
  return [...bounded, { stage, action: canonical, dismissed: Boolean(dismissed), conceptSurfaced: Boolean(conceptSurfaced) }].slice(-12);
}
